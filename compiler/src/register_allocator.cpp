...
std::string RegisterAllocator::get_sized_reg(const std::string& phys, const std::string& current_op, const InstructionSet::Instruction& instr) {
    ...
    if (sub_regs.find(phys) == sub_regs.end()) return phys;

    if (current_op.find("[") != std::string::npos) return sub_regs.at(phys)[2]; 

    std::string m = instr.mnemonic;
    if (m.find("movsx") == 0 || m.find("movsxd") == 0 || m.find("lea") == 0) return sub_regs.at(phys)[2];
    ...
    for (int i = 0; i < (int)instructions.size(); ++i) {
        ...
        for (auto it = vreg_to_phys.begin(); it != vreg_to_phys.end(); ) {
            if (intervals[it->first].end < i) {
                if (it->second.find("xmm") == 0) xmm_pool.push_back(it->second);
                else int_pool.push_back(it->second);
        ...
        for (auto& op : instr.operands) {
            ...
            while (std::regex_search(search_str, m, e)) {
                ...
                if (spill_slots.count(vreg) && vreg_to_phys.find(vreg) == vreg_to_phys.end()) {
                    ...
                    std::string reg = pool.back(); pool.pop_back();
                    vreg_to_phys[vreg] = reg;
                    physical_instrs.push_back({(reg.find("xmm") == 0 ? "vmovss" : "mov"), {reg, spill_slots[vreg]}});
                }
                ...
                if (vreg_to_phys.find(vreg) == vreg_to_phys.end()) {
                    ...
                    if (pool.empty()) {
                        ...
                        next_spill_offset += 8;
                        spill_slots[victim_v] = slot;
                        physical_instrs.push_back({(victim_p.find("xmm") == 0 ? "vmovss" : "mov"), {slot, victim_p}});
                        vreg_to_phys.erase(victim_v);
                        pool.push_back(victim_p);
                    }
                    vreg_to_phys[vreg] = pool.back(); pool.pop_back();
                }
