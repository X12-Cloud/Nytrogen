      ...
      if (decl.initial_value) {
        if (decl.initial_value->is_constant()) {
          if (is_string) {
            ...
            constants.push_back(
                {str_data_label, "db",
                 "\"" + unescapeString(decl.initial_value->get_value()) +
                     "\", 0"});
...
void CodeGenerator::visit(VariableReferenceNode *node) {
  ...
  if (is_lvalue) {
    ...
    emitter.load_adv(size, node->resolved_type.get(),
                     (is_float || is_double) ? "xmm0" : "rax", "rbp", offset);
  }
}
...
void CodeGenerator::visit(BinaryOperationExpressionNode *node) {
  ...
  if (debug_mode) {
    if (node->resolved_type->category == TypeNode::TypeCategory::PRIMITIVE) {
      auto prim =
          std::static_pointer_cast<PrimitiveTypeNode>(node->resolved_type);
      std::cout << "Debug: Primitive Type ID found: " << prim->primitive_type
                << std::endl;
      std::cout << "Debug: Expected FLOAT_LITERAL: " << Token::FLOAT_LITERAL
                << std::endl;
      std::cout << "Debug: Expected KEYWORD_FLOAT: " << Token::KEYWORD_FLOAT
                << std::endl;
    }
  }
...
void CodeGenerator::visit(ReturnStatementNode* node) {
    if (node->expression) {
        ...
        if (var_ref && var_ref->resolved_symbol && var_ref->resolved_symbol->offset < 0) {
            ...
            std::string reg = isFloatingPoint(var_ref->resolved_type.get()) ? "xmm0" : "rax";
...
void CodeGenerator::visit(FunctionCallNode *node) {
    ...
    if (node->resolved_symbol == nullptr) {
        throw std::runtime_error("CodeGen Error: Function " + node->function_name +
                                 " not found.");
    }
    ...
    for (int i = 0; i < std::min(arg_count, 6); ++i) {
        ...
        if (isFloatingPoint(node->arguments[i]->resolved_type.get())) {
            if (xmm_reg_idx < 8) {
                if (xmm_reg_idx != 0) {
                    emitter.emit("movsd", "xmm" + std::to_string(xmm_reg_idx), "xmm0");
                }
                xmm_reg_idx++;
            }
        } else {
...
void CodeGenerator::visit(ArrayAccessNode *node) {
  ...
  if (node->array_expr && node->array_expr->resolved_type) {
    if (node->array_expr->resolved_type->category ==
      ...
      auto arr_type =
          static_cast<ArrayTypeNode *>(node->array_expr->resolved_type.get());
