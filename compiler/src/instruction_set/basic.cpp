...
void InstructionSet::mov_indirect(const std::string& base, int offset, const std::string& src) {
    std::string addr = "[" + base + (offset >= 0 ? " + " : " - ") + std::to_string(std::abs(offset)) + "]";
    emit("mov", addr, src);
}
...
void InstructionSet::emit_load_constant(const std::string& instr, const std::string& dest, const std::string& label) {
